...
pub enum HeadEvents<'a, MapType: Map>{
    MOVE_HEAD{direction : Option<Direction>, map : &'a mut MapType},
...
pub trait Head : private::Sealed {
    fn new(id: Id, position: Coordinates, coming_from: Direction, events_sender : Sender<BoardEvevents>) -> SimpleHead;
    fn dispatch(&mut self, event : HeadEvents<impl Map>);
}
mod private{
    use crate::{utils::{Direction, Coordinates, DirectionFlags}, map::Map};
    ...
    pub trait Sealed {
        ...
        fn move_and_mark_tile(&mut self, position: Coordinates, direction : Direction, map : &mut impl Map) -> HeadAction;
        ...
        fn try_explore_direction(&mut self, choosen_direction : Direction, dir_flags: &mut DirectionFlags, map: &mut impl Map) ->  Direction;
...
impl private::Sealed for SimpleHead{
    ...
    fn move_head_handler(&mut self, direction : Option<Direction>,  map : &mut impl Map) {
        ...
        match new_tile_type {
            TileType::Separator => {
                let add_head_event = BoardEvevents::ADD_HEAD { position: head_original_position, coming_from: head_original_provenance, parent_direction: chosen_direction};
...
impl Head for SimpleHead {
    fn new(id: Id, position: Coordinates, coming_from: Direction, events_sender : Sender<BoardEvevents>) -> SimpleHead {
        SimpleHead{id, position, coming_from, events_sender}
    }

    fn dispatch(&mut self, event : HeadEvents<impl Map>) {
        match event {
            HeadEvents::MOVE_HEAD { direction, map } => private::Sealed::move_head_handler(self, direction, map),
        };
    }
}
